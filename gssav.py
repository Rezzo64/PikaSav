...
class GSSav:
    ...
    def __init__(self, file, repair = False):
        ...
        self.ok = repair
        self.repair = repair
        self.maps = [('Main', 53665, 8201, 11624)]
        table = [''] * 256
        etable = [''] * 256
        for x in range(26):
    ...
    def check_sav(self):
        ...
        size = 3424
        checksum = 0
        self.checksum = self.buffer[11625]
        self.checksum += self.buffer[11626] * 256
        for x in range(size):
        ...
        self.setbyte(11625, checksum & 255)
        self.setbyte(11626, checksum >> 8 & 255)
    ...
    def set(self, var, value):
        ...
        if var == 'money':
            self.setbyte(9179, int(value) >> 16 & 255)
            self.setbyte(9180, int(value) >> 8 & 255)
            self.setbyte(9181, int(value) & 255)
        if var == 'chips':
            self.setbyte(9186, int(value) >> 8)
            self.setbyte(9187, int(value) & 255)
        if var == 'hours':
            self.setbyte(8276, int(value) & 255)
            self.setbyte(8275, int(value) >> 8)
        if var == 'minutes':
            self.setbyte(8277, int(value))
        if var == 'seconds':
            self.setbyte(8279, int(value))
        if var == 'itemcount':
            self.setbyte(9247, int(value))
        if var == 'pcitemcount':
            self.setbyte(9342, int(value))
        if var == 'pokemoncount':
            self.setbyte(10378, int(value))
        for b in range(14):
    ...
    def load_time(self):
        self.hours = self.buffer[8276]
        self.hours += self.buffer[8275] * 256
        self.minutes = self.buffer[8277]
        self.seconds = self.buffer[8279]
    ...
    def setpokedex(self, x, isseen, iscatched):
        pos = 2 ** ((x - 1) % 8)
        seen = self.buffer[10828 + (x - 1) // 8]
        catched = self.buffer[10860 + (x - 1) // 8]
        if self.seen[x] != isseen:
            seen ^= pos
        if self.catched[x] != iscatched:
            catched ^= pos
        self.setbyte(10828 + (x - 1) // 8, seen)
        self.setbyte(10860 + (x - 1) // 8, catched)

    def load_pokedex(self):
        self.seen = [0] * 257
        self.catched = [0] * 257
        for x in range(32):
            catched = self.buffer[10828 + x]
            seen = self.buffer[10860 + x]
            for y in range(8):
    ...
    def load_items(self):
        items = [[0, 0]] * 20
        for x in range(20):
            item = self.buffer[9248 + 2 * x]
            count = self.buffer[9249 + 2 * x]
            items[x] = [item, count]

        self.items = items
        self.itemcount = self.buffer[9247]
        pcitems = [[0, 0]] * 50
        for x in range(50):
            item = self.buffer[9343 + 2 * x]
            count = self.buffer[9344 + 2 * x]
            pcitems[x] = [item, count]

        self.pcitems = pcitems
        self.pcitemcount = self.buffer[9342]

    def load_pokemon(self):
        self.pokemon = [''] * 6
        self.pcpokemon = [''] * 280
        self.pokemoncount = self.buffer[10378]
        self.boxpokemoncount = [0] * 14
        self.boxoffset = [0] * 14
        for p in range(6):
            self.pokemon[p] = self.pkm(10379 + p, 10674 + 11 * p, 10740 + 11 * p, 10386 + p * 48)

        self.currentbox = self.buffer[10020] & 127
        for b in range(14):
    ...
    def setpokemon(self, p, pkm):
        self.pkm(10379 + p, 10674 + 11 * p, 10740 + 11 * p, 10386 + p * 48, pkm)
    ...
    def setitem(self, x, item, count):
        if x < 20:
            self.setbyte(9248 + 2 * x, item)
            self.setbyte(9249 + 2 * x, count)
        if x >= 20:
            self.setbyte(9343 + 2 * (x - 20), item)
            self.setbyte(9344 + 2 * (x - 20), count)

    def load_money(self):
        money = 0
        money += self.buffer[9179] * 65536
        money += self.buffer[9180] * 256
        money += self.buffer[9181]
        chips = 0
        chips +=self.buffer[9186] * 256
        chips += self.buffer[9187]
        self.money = money
        self.chips = chips
